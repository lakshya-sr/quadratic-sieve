...
@profile
def main():
    global M, base, b, needed
    if(len(sys.argv) > 1): N = int(sys.argv[1])
    else: N = 836858951270212201
    sieve_start = 0
    sieve_stop = 0
    bound = round((math.e**(math.sqrt(math.log(N)*math.log(math.log(N)))))**(math.sqrt(2)/4))
    a = 1
    b = isqrt(N) + 1
    sieve_interval = bound**3
    base = create_base(N, bound)
    start_vals = solve(a, b, N)
    seen = set([])
    needed = phi(base[-1]) + 1
    print(f"bound = {bound}, a = {a}, b = {b}, needed = {needed}, sieve_interval = {sieve_interval}")
    ...
    while len(smooth_vals) < needed:
      ...
      interval = [((a * x + b) ** 2) - N for x in range(sieve_start, sieve_stop)]
      ...
      for p in range(len(base)):
        ...
        while r1 < sieve_stop:
          while interval[r1 - sieve_start] % base[p] == 0:
            interval[r1 - sieve_start] /= base[p]
          r1 += base[p]

        # Sieve the values using both start positions if they aren't equal
        interval[r2 - sieve_start] /= base[p]
        if r2 != start_vals[p][0]:
          while r2 < sieve_stop:
            while interval[r2 - sieve_start] % base[p] == 0:
              interval[r2 - sieve_start] /= base[p]
            r2 += base[p]
